package com.rodrigodev.xgen4j.test.info_generation.eb1.eb2.eb3_3;

import com.rodrigodev.xgen4j.test.info_generation.eb1.eb2.EB2Exception;
import com.rodrigodev.xgen4j.test.info_generation.RootException;

/**
 * Autogenerated by XGen on January 1, 0001.
 */
public class EB3_3Exception extends EB2Exception {

    public static final ExceptionType TYPE = new ExceptionType();

    private static class ExceptionType extends RootException.ExceptionType {

        @Override
        protected RootException createException(String message) {
            return new EB3_3Exception(message);
        }

        @Override
        protected RootException createException(String message, Throwable cause) {
            return new EB3_3Exception(message, cause);
        }
    }

    protected EB3_3Exception(String message) {
        super(message);
    }

    protected EB3_3Exception(String message, Throwable cause) {
        super(message, cause);
    }
}
